...
aoi_definitions = {
    'Sender Address': {'x_range': (460, 710), 'y_range': (255, 300)},
...
# Define consistent colors for AOIs
aoi_colors = {
    'Sender Address': 'orange',
    'Email Header': 'blue',
    'Main Content': 'green',
    'Footer': 'red',
    'Inbox and Folders': 'purple',
    'Other': 'grey'
}
...
for email_index in range(12):
    ...
    sns.scatterplot(
        data=fixations, 
        x=fixations['Average eye position (pixels)'].apply(lambda p: p[0]), 
        y=fixations['Average eye position (pixels)'].apply(lambda p: p[1]), 
        hue='aoi', 
        palette=aoi_colors,  # Use consistent colors
        ax=ax
    )
    ax.xaxis.set_label_position('top')
    ax.xaxis.tick_top()
    plt.legend(loc='lower left')  # Place legend in the bottom left
    ...
    aoi_counts = df_email['aoi'].value_counts().reindex(aoi_definitions.keys(), fill_value=0).reset_index()
    ...
    sns.barplot(
        ...
        x='AOI', 
        y='Count', 
        palette=[aoi_colors[aoi] for aoi in aoi_definitions.keys()]
    )
